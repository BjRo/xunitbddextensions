...
namespace Xunit.Reporting.Core
{
    ...
    {
        private static string lastAssemblyLoadPath;
        private static readonly string[] knownExtensions = new[] { ".dll", ".exe" };
        ...
        public IAssembly Load(string assemblyName)
        {
            ...
            if (NameContainsFileExtension(assemblyName))
            {
                ...
                {               
                    ...
                    lastAssemblyLoadPath = Path.GetDirectoryName(assemblyName);
                }
            }
            else
            {
        ...
        {
            if (string.IsNullOrEmpty(lastAssemblyLoadPath))
            {
                return null;
            }
        ...
        private static System.Reflection.Assembly TryToLoadFromLastLoadPath(ResolveEventArgs args)
        {
            ...
            {
                var updatedLoadPath = BuildFullNameWithLastLoadDirectory(nameWithoutVersion, extension);

                if (!File.Exists(updatedLoadPath))
                {
                    continue;
                }

                return System.Reflection.Assembly.LoadFile(updatedLoadPath);
            }
