...
namespace Xunit
{
    ...
    public abstract class ObservationAttributeBase : FactAttribute
    {
        ...
        /// </returns>
        protected override sealed IEnumerable<ITestCommand> EnumerateTestCommands(IMethodInfo method)
        {
            return
                base.EnumerateTestCommands(method).Select(command => new SpecificationCommand(command, method)).Cast
                    <ITestCommand>();
        }
    }
}